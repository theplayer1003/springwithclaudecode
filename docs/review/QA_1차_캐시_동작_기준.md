...
// 2. save 후 조회
postRepository.save(newPost);              // INSERT + 영속 상태로 등록
...
1차 캐시는 **ID를 키로 하는 Map**이므로, **ID 기반 조회**에서만 캐시 히트가 발생한다. `findById()`만 해당되는 것이 아니라, save, findAll 등 어떤 경로로든 영속 상태가 된 엔티티는 이후 ID 조회 시 캐시에서 반환된다.
