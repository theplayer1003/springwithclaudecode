...
Controller의 `ResponseEntity`도 결국 Spring이 내부적으로 `response.setStatus()`, `response.getWriter().write()` 등을 호출한다. Filter에서는 Spring MVC 계층 밖이라 직접 해야 할 뿐이다.
...
**A:** `@RestControllerAdvice`는 Controller 레벨의 예외만 잡는다. Filter는 Controller보다 앞단에서 동작하므로, Filter에서 발생한 예외는 `@RestControllerAdvice`의 범위 밖이다.
