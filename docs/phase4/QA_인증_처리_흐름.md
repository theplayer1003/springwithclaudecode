...
**A:** SecurityConfig의 `authorizeHttpRequests` 규칙을 기반으로 `FilterSecurityInterceptor`가 처리한다. 이 필터는 Security Filter Chain의 마지막쯤에 위치한다.
...
**A:** 클라이언트 요청에 생성하고자 하는 리소스 정보와 함께 토큰이 오면, 이 요청이 컨트롤러에 도달하기 전 Spring Security 필터 체인이 토큰 유효성을 검증하고 정보를 추출해 "인증되었음"을 SecurityContext에 등록한다. 그 후 인가 규칙을 통과한 요청만 컨트롤러에 도달하며, 컨트롤러는 SecurityContext에서 인증 결과물만 꺼내 비즈니스 로직을 수행한다.
